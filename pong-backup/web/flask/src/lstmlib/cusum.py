...
def detect_cumsum_realtime(currentvalue,previousvalue,previousgp,previousgn, threshold=1, drift=0):
    alarm = False
    s = currentvalue - previousvalue
    gp = previousgp + s - drift  # cumulative sum for + change
    gn = previousgn - s - drift  # cumulative sum for - change
    if gp < 0:
        gp = 0
    if gn < 0:
        gn= 0
    if gp > threshold or gn > threshold:  # change detected!
        alarm = True
        gp, gn = 0, 0      # reset alarm
    else:
        alarm = False
    return alarm,gp,gn
def _plot(x, threshold, drift, ending, ax, ta, tai, taf, gp, gn):
    ...
    try:
        ...
        if ax is None:
            _, (ax1,ax2) = plt.subplots(2, 1, figsize=(28, 6))#(ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))
