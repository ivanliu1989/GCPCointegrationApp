...
def approach2(df,pvalue,datefrom,dateto):
    ...
    if p <= pvalue:
        ...
        return {'instrument1':c1,'instrument2':c2,'from':datefrom,'to':dateto,'public_url':public_url,'hedge_ratio':str(round(hedgingratio,3)),'coeff1':str(vec[0]),'coeff2':str(vec[1]),'p-value':str(round(p,3)),'mean':str(round(mean,6)),'std':str(round(std,6))}
    return None
