...
def lstmchart():
	...
	#instrument = 'EUR_USD'
	df = getpair.getlstmdata(instrument)
	tuples = [tuple(x) for x in df[['time','actual_value']].values]
	tuples2 = [tuple(x) for x in df[['time','predicted_value']].values]
	
	graph = pygal.DateTimeLine(
	    x_label_rotation=0, truncate_label=-1,spacing=0,height=250,
	    x_value_formatter=lambda dt: dt.strftime('%Y-%m-%d %H:%M:%S'), style=DarkStyle)
	graph.add(instrument, tuples, show_dots=True, dots_size=2, stroke=True)
	graph.add('predicted value', tuples2, show_dots=True, dots_size=2, stroke=False)
...
def createnvd3chartlist(n,instrument1,instrument2,display):
	...
	for i in range(n):
		...
		df2 = df2.dropna()
		columns = ['id','instrument','time','predict','public_url','coeff1','coeff2']
...
def createnvd3chart(id,df,df2,dfforecast,instrument1,instrument2,datefrom,dateto,display):

	if len(df)==0:
		return ''
	df[instrument1] = savechart.normalise_windows(df[instrument1],df[instrument1].iloc[0])
	df.rename(columns={'time': 'x', instrument1: 'y'}, inplace=True)
	...
	for i in range(len(dfforecast)):
		...
		d = df[df['x']<t]
		value.append(d.tail(1)['y'].iloc[0])

	dfforecast['y'] = value
	dfforecast.rename(columns={'time': 'x'}, inplace=True)
	data3 = dfforecast[['x','y']].to_dict(orient='records')
	...
	<script>
		dict%id = %data4
		dictid%id = %data5
		chartdata%id =[{"key": "%serie1", "yAxis": "1", "values": %data1},{"key": "%serie2", "yAxis": "1", "values": %data2},{"key": "cointegration", "yAxis": "1", "color": "red", "values": %data3}];
		...
		chart.xDomain([new Date(%datefrom),new Date(%dateto)]);
	...
	html = html.replace('%data3',str(data3))
